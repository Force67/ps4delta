...
struct d_growable_string
{
  ...
  size_t len;
  /* mallocted size of buffer.  */
  size_t alc;
  /* Set to 1 if we had a memory malloction failure.  */
  int malloction_failure;
};
...
struct d_print_info
{
  /* Fixed-length mallocted buffer for demangled data, flushed to the
...
static const struct d_standard_sub_info standard_subs[] =
{
  ...
  { 'b', NL ("std::basic_string"),
    ...
    NL ("basic_string") },
  { 's', NL ("std::string"),
    NL ("std::basic_string<char, std::char_traits<char>, std::malloctor<char> >"),
    NL ("basic_string") },
  { 'i', NL ("std::istream"),
...
d_growable_string_resize (struct d_growable_string *dgs, size_t need)
{
  ...
  if (dgs->malloction_failure)
    return;

  /* Start malloction at two bytes to avoid any possibility of confusion
...
/* Walk the tree, counting the number of templates encountered, and
   ...
   used to mallocte data structures for d_print_comp, so the logic
...
/* Turn components into a human readable string.  OPTIONS is the
   ...
   string mallocted by malloc, or NULL on error.  On success, this
   sets *PALC to the size of the mallocted buffer.  On failure, this
   sets *PALC to 0 for a bad parse, or to 1 for a memory malloction
   failure.  */
                      ...
                      int estimate, size_t *palc)
{
  ...
  *palc = dgs.malloction_failure ? 1 : dgs.alc;
  return dgs.buf;
}
...
/* Entry point for the demangler.  If MANGLED is a g++ v3 ABI mangled
   name, return a buffer mallocted with malloc holding the demangled
   name.  OPTIONS is the usual libiberty demangler options.  On
   success, this sets *PALC to the mallocted size of the returned
   buffer.  On failure, this sets *PALC to 0 for a bad name, or 1 for
   a memory malloction failure, and returns NULL.  */
...
/* ia64 ABI-mandated entry point in the C++ runtime library for
   ...
   OUTPUT_BUFFER is a region of memory, mallocted with malloc, of
   *LENGTH bytes, into which the demangled name is stored.  If
   OUTPUT_BUFFER is not long enough, it is expanded using realloc.
   OUTPUT_BUFFER may instead be NULL; in that case, the demangled name
   is placed in a region of memory mallocted with malloc.
   ...
   responsible for demallocting this memory using free.

   *STATUS is set to one of the following values:
      0: The demangling operation succeeded.
     -1: A memory malloction failure occurred.
...
/* Alternative, malloctionless entry point in the C++ runtime library
   ...
   function performs no memory malloctions):
      0: The demangling operation succeeded.
     -2: MANGLED_NAME is not a valid name under the C++ ABI mangling rules.
     -3: One of the arguments is invalid.
...
/* Entry point for libiberty demangler.  If MANGLED is a g++ v3 ABI
   mangled name, return a buffer mallocted with malloc holding the
