#pragma once

// Copyright (C) Force67

// This file was generated on 08/12/2019

#include "../../vmodlinker.h"

int PS4ABI __elf_phdr_match_addr();
int PS4ABI __error();
int PS4ABI __getcwd();
int PS4ABI __inet_ntop();
int PS4ABI __inet_pton();
int PS4ABI __progname();
int PS4ABI __pthread_cleanup_pop_imp();
int PS4ABI __pthread_cleanup_push_imp();
int PS4ABI __pthread_cxa_finalize();
int PS4ABI __sceKernelGetGPI();
int PS4ABI __stack_chk_fail();
int PS4ABI __stack_chk_guard();
int PS4ABI __sys_debug_init();
int PS4ABI __sys_nanosleep();
int PS4ABI __tls_get_addr();
int PS4ABI __Ux86_64_setcontext();
int PS4ABI _accept();
int PS4ABI _bind();
int PS4ABI _close();
int PS4ABI _connect();
int PS4ABI _dup2();
int PS4ABI _execve();
int PS4ABI _execvpe();
int PS4ABI _exit();
int PS4ABI _fcntl();
int PS4ABI _fpathconf();
int PS4ABI _fstat();
int PS4ABI _fstatfs();
int PS4ABI _getdirentries();
int PS4ABI _getpeername();
int PS4ABI _getsockname();
int PS4ABI _getsockopt();
int PS4ABI _ioctl();
int PS4ABI _is_signal_return();
int PS4ABI _listen();
int PS4ABI _nanosleep();
int PS4ABI _open();
int PS4ABI _openat();
int PS4ABI _read();
int PS4ABI _readv();
int PS4ABI _recvfrom();
int PS4ABI _recvmsg();
int PS4ABI _sceKernelRtldSetApplicationHeapAPI();
int PS4ABI _sendmsg();
int PS4ABI _sendto();
int PS4ABI _setsockopt();
int PS4ABI _sigaction();
int PS4ABI _sigintr();
int PS4ABI _sigprocmask();
int PS4ABI _sigsuspend();
int PS4ABI _umtx_op();
int PS4ABI _wait4();
int PS4ABI _write();
int PS4ABI _writev();
int PS4ABI accept();
int PS4ABI access();
int PS4ABI aio_cancel();
int PS4ABI aio_error();
int PS4ABI aio_fsync();
int PS4ABI aio_read();
int PS4ABI aio_return();
int PS4ABI aio_suspend();
int PS4ABI aio_waitcomplete();
int PS4ABI aio_write();
int PS4ABI amd64_set_fsbase();
int PS4ABI bind();
int PS4ABI blockpool_batch();
int PS4ABI blockpool_map();
int PS4ABI blockpool_open();
int PS4ABI blockpool_unmap();
int PS4ABI chdir();
int PS4ABI chflags();
int PS4ABI chmod();
int PS4ABI chown();
int PS4ABI chroot();
int PS4ABI clock_getres();
int PS4ABI clock_gettime();
int PS4ABI clock_settime();
int PS4ABI close();
int PS4ABI connect();
int PS4ABI cpuset();
int PS4ABI cpuset_getaffinity();
int PS4ABI cpuset_getid();
int PS4ABI cpuset_setaffinity();
int PS4ABI cpuset_setid();
int PS4ABI creat();
int PS4ABI dlclose();
int PS4ABI dlerror();
int PS4ABI dlopen();
int PS4ABI dlsym();
int PS4ABI dup();
int PS4ABI dup2();
int PS4ABI dynlib_get_obj_member();
int PS4ABI environ();
int PS4ABI errno();
int PS4ABI execv();
int PS4ABI execve();
int PS4ABI execvp();
int PS4ABI fchdir();
int PS4ABI fchflags();
int PS4ABI fchmod();
int PS4ABI fchmodat();
int PS4ABI fchown();
int PS4ABI fchownat();
int PS4ABI fcntl();
int PS4ABI fdatasync();
int PS4ABI flock();
int PS4ABI fork();
int PS4ABI fpathconf();
int PS4ABI fstat();
int PS4ABI fstatat();
int PS4ABI fstatfs();
int PS4ABI fsync();
int PS4ABI ftruncate();
int PS4ABI futimes();
int PS4ABI futimesat();
int PS4ABI get_authinfo();
int PS4ABI get_sdk_compiled_version();
int PS4ABI get_self_auth_info();
int PS4ABI get_vm_map_timestamp();
int PS4ABI getargc();
int PS4ABI getargv();
int PS4ABI getcontext();
int PS4ABI getdents();
int PS4ABI getdirentries();
int PS4ABI getdtablesize();
int PS4ABI getegid();
int PS4ABI geteuid();
int PS4ABI getfsstat();
int PS4ABI getgid();
int PS4ABI getgroups();
int PS4ABI getitimer();
int PS4ABI getlogin();
int PS4ABI getlogin_r();
int PS4ABI getpagesize();
int PS4ABI getpeername();
int PS4ABI getpid();
int PS4ABI getppid();
int PS4ABI getpriority();
int PS4ABI getrlimit();
int PS4ABI getrusage();
int PS4ABI getsid();
int PS4ABI getsockname();
int PS4ABI getsockopt();
int PS4ABI gettimeofday();
int PS4ABI getuid();
int PS4ABI htonl();
int PS4ABI htons();
int PS4ABI inet_ntop();
int PS4ABI inet_pton();
int PS4ABI ioctl();
int PS4ABI ipmimgr_call();
int PS4ABI is_in_sandbox();
int PS4ABI issetugid();
int PS4ABI kenv();
int PS4ABI kevent();
int PS4ABI kill();
int PS4ABI kldfind();
int PS4ABI kldfirstmod();
int PS4ABI kldload();
int PS4ABI kldnext();
int PS4ABI kldstat();
int PS4ABI kldsym();
int PS4ABI kldunload();
int PS4ABI kmq_notify();
int PS4ABI kmq_open();
int PS4ABI kmq_setattr();
int PS4ABI kmq_timedreceive();
int PS4ABI kmq_timedsend();
int PS4ABI kmq_unlink();
int PS4ABI kqueue();
int PS4ABI ksem_close();
int PS4ABI ksem_destroy();
int PS4ABI ksem_getvalue();
int PS4ABI ksem_init();
int PS4ABI ksem_open();
int PS4ABI ksem_post();
int PS4ABI ksem_timedwait();
int PS4ABI ksem_trywait();
int PS4ABI ksem_unlink();
int PS4ABI ksem_wait();
int PS4ABI ktimer_create();
int PS4ABI ktimer_delete();
int PS4ABI ktimer_getoverrun();
int PS4ABI ktimer_gettime();
int PS4ABI ktimer_settime();
int PS4ABI ktrace();
int PS4ABI lchflags();
int PS4ABI lchmod();
int PS4ABI lchown();
int PS4ABI link();
int PS4ABI linkat();
int PS4ABI listen();
int PS4ABI lseek();
int PS4ABI lstat();
int PS4ABI lutimes();
int PS4ABI madvise();
int PS4ABI mdbg_call();
int PS4ABI mdbg_service();
int PS4ABI mincore();
int PS4ABI mkdir();
int PS4ABI mkdirat();
int PS4ABI mlock();
int PS4ABI mlockall();
int PS4ABI mmap();
int PS4ABI mount();
int PS4ABI mprotect();
int PS4ABI msync();
int PS4ABI munlock();
int PS4ABI munlockall();
int PS4ABI munmap();
int PS4ABI nanosleep();
int PS4ABI nmount();
int PS4ABI ntohl();
int PS4ABI ntohs();
int PS4ABI open();
int PS4ABI openat();
int PS4ABI pathconf();
int PS4ABI pause();
int PS4ABI physhm_open();
int PS4ABI physhm_unlink();
int PS4ABI pipe();
int PS4ABI poll();
int PS4ABI pread();
int PS4ABI preadv();
int PS4ABI profil();
int PS4ABI pselect();
int PS4ABI pthread_atfork();
int PS4ABI pthread_attr_destroy();
int PS4ABI pthread_attr_get_np();
int PS4ABI pthread_attr_getaffinity_np();
int PS4ABI pthread_attr_getdetachstate();
int PS4ABI pthread_attr_getguardsize();
int PS4ABI pthread_attr_getinheritsched();
int PS4ABI pthread_attr_getschedparam();
int PS4ABI pthread_attr_getschedpolicy();
int PS4ABI pthread_attr_getscope();
int PS4ABI pthread_attr_getstack();
int PS4ABI pthread_attr_getstackaddr();
int PS4ABI pthread_attr_getstacksize();
int PS4ABI pthread_attr_init();
int PS4ABI pthread_attr_setaffinity_np();
int PS4ABI pthread_attr_setcreatesuspend_np();
int PS4ABI pthread_attr_setdetachstate();
int PS4ABI pthread_attr_setguardsize();
int PS4ABI pthread_attr_setinheritsched();
int PS4ABI pthread_attr_setschedparam();
int PS4ABI pthread_attr_setschedpolicy();
int PS4ABI pthread_attr_setscope();
int PS4ABI pthread_attr_setstack();
int PS4ABI pthread_attr_setstackaddr();
int PS4ABI pthread_attr_setstacksize();
int PS4ABI pthread_barrier_destroy();
int PS4ABI pthread_barrier_init();
int PS4ABI pthread_barrier_wait();
int PS4ABI pthread_barrierattr_destroy();
int PS4ABI pthread_barrierattr_getpshared();
int PS4ABI pthread_barrierattr_init();
int PS4ABI pthread_barrierattr_setpshared();
int PS4ABI pthread_cancel();
int PS4ABI pthread_cleanup_pop();
int PS4ABI pthread_cleanup_push();
int PS4ABI pthread_cond_broadcast();
int PS4ABI pthread_cond_destroy();
int PS4ABI pthread_cond_init();
int PS4ABI pthread_cond_signal();
int PS4ABI pthread_cond_signalto_np();
int PS4ABI pthread_cond_timedwait();
int PS4ABI pthread_cond_wait();
int PS4ABI pthread_condattr_destroy();
int PS4ABI pthread_condattr_getclock();
int PS4ABI pthread_condattr_getpshared();
int PS4ABI pthread_condattr_init();
int PS4ABI pthread_condattr_setclock();
int PS4ABI pthread_condattr_setpshared();
int PS4ABI pthread_create();
int PS4ABI pthread_create_name_np();
int PS4ABI pthread_detach();
int PS4ABI pthread_equal();
int PS4ABI pthread_exit();
int PS4ABI pthread_get_user_context_np();
int PS4ABI pthread_getaffinity_np();
int PS4ABI pthread_getconcurrency();
int PS4ABI pthread_getcpuclockid();
int PS4ABI pthread_getname_np();
int PS4ABI pthread_getprio();
int PS4ABI pthread_getschedparam();
int PS4ABI pthread_getspecific();
int PS4ABI pthread_getthreadid_np();
int PS4ABI pthread_join();
int PS4ABI pthread_key_create();
int PS4ABI pthread_key_delete();
int PS4ABI pthread_kill();
int PS4ABI pthread_main_np();
int PS4ABI pthread_multi_np();
int PS4ABI pthread_mutex_destroy();
int PS4ABI pthread_mutex_getprioceiling();
int PS4ABI pthread_mutex_getspinloops_np();
int PS4ABI pthread_mutex_getyieldloops_np();
int PS4ABI pthread_mutex_init();
int PS4ABI pthread_mutex_isowned_np();
int PS4ABI pthread_mutex_lock();
int PS4ABI pthread_mutex_reltimedlock_np();
int PS4ABI pthread_mutex_setname_np();
int PS4ABI pthread_mutex_setprioceiling();
int PS4ABI pthread_mutex_setspinloops_np();
int PS4ABI pthread_mutex_setyieldloops_np();
int PS4ABI pthread_mutex_timedlock();
int PS4ABI pthread_mutex_trylock();
int PS4ABI pthread_mutex_unlock();
int PS4ABI pthread_mutexattr_destroy();
int PS4ABI pthread_mutexattr_getkind_np();
int PS4ABI pthread_mutexattr_getprioceiling();
int PS4ABI pthread_mutexattr_getprotocol();
int PS4ABI pthread_mutexattr_getpshared();
int PS4ABI pthread_mutexattr_gettype();
int PS4ABI pthread_mutexattr_init();
int PS4ABI pthread_mutexattr_setkind_np();
int PS4ABI pthread_mutexattr_setprioceiling();
int PS4ABI pthread_mutexattr_setprotocol();
int PS4ABI pthread_mutexattr_setpshared();
int PS4ABI pthread_mutexattr_settype();
int PS4ABI pthread_once();
int PS4ABI pthread_rename_np();
int PS4ABI pthread_resume_all_np();
int PS4ABI pthread_resume_np();
int PS4ABI pthread_rwlock_destroy();
int PS4ABI pthread_rwlock_init();
int PS4ABI pthread_rwlock_rdlock();
int PS4ABI pthread_rwlock_reltimedrdlock_np();
int PS4ABI pthread_rwlock_reltimedwrlock_np();
int PS4ABI pthread_rwlock_timedrdlock();
int PS4ABI pthread_rwlock_timedwrlock();
int PS4ABI pthread_rwlock_tryrdlock();
int PS4ABI pthread_rwlock_trywrlock();
int PS4ABI pthread_rwlock_unlock();
int PS4ABI pthread_rwlock_wrlock();
int PS4ABI pthread_rwlockattr_destroy();
int PS4ABI pthread_rwlockattr_getpshared();
int PS4ABI pthread_rwlockattr_gettype_np();
int PS4ABI pthread_rwlockattr_init();
int PS4ABI pthread_rwlockattr_setpshared();
int PS4ABI pthread_rwlockattr_settype_np();
int PS4ABI pthread_self();
int PS4ABI pthread_set_name_np();
int PS4ABI pthread_set_user_context_np();
int PS4ABI pthread_setaffinity_np();
int PS4ABI pthread_setcancelstate();
int PS4ABI pthread_setcanceltype();
int PS4ABI pthread_setconcurrency();
int PS4ABI pthread_setprio();
int PS4ABI pthread_setschedparam();
int PS4ABI pthread_setspecific();
int PS4ABI pthread_sigmask();
int PS4ABI pthread_single_np();
int PS4ABI pthread_spin_destroy();
int PS4ABI pthread_spin_init();
int PS4ABI pthread_spin_lock();
int PS4ABI pthread_spin_trylock();
int PS4ABI pthread_spin_unlock();
int PS4ABI pthread_suspend_all_np();
int PS4ABI pthread_suspend_np();
int PS4ABI pthread_suspend_user_context_np();
int PS4ABI pthread_switch_add_np();
int PS4ABI pthread_switch_delete_np();
int PS4ABI pthread_testcancel();
int PS4ABI pthread_timedjoin_np();
int PS4ABI pthread_yield();
int PS4ABI ptrace();
int PS4ABI pwrite();
int PS4ABI pwritev();
int PS4ABI raise();
int PS4ABI read();
int PS4ABI readlink();
int PS4ABI readv();
int PS4ABI reboot();
int PS4ABI recv();
int PS4ABI recvfrom();
int PS4ABI recvmsg();
int PS4ABI rename();
int PS4ABI renameat();
int PS4ABI revoke();
int PS4ABI rfork_thread();
int PS4ABI rmdir();
int PS4ABI rtld_printf();
int PS4ABI rtprio_thread();
int PS4ABI sceKernelAddFileEvent();
int PS4ABI sceKernelAddHRTimerEvent();
int PS4ABI sceKernelAddReadEvent();
int PS4ABI sceKernelAddTimerEvent();
int PS4ABI sceKernelAddUserEvent();
int PS4ABI sceKernelAddUserEventEdge();
int PS4ABI sceKernelAddWriteEvent();
int PS4ABI sceKernelAllocateDirectMemory();
int PS4ABI sceKernelAllocateMainDirectMemory();
int PS4ABI sceKernelAvailableDirectMemorySize();
int PS4ABI sceKernelAvailableFlexibleMemorySize();
int PS4ABI sceKernelBacktraceSelf();
int PS4ABI sceKernelBatchMap();
int PS4ABI sceKernelBatchMap2();
int PS4ABI sceKernelCancelEventFlag();
int PS4ABI sceKernelCancelSema();
int PS4ABI sceKernelCheckedReleaseDirectMemory();
int PS4ABI sceKernelCheckReachability();
int PS4ABI sceKernelChmod();
int PS4ABI sceKernelClearEventFlag();
int PS4ABI sceKernelClearGameDirectMemory();
int PS4ABI sceKernelClockGetres();
int PS4ABI sceKernelClockGettime();
int PS4ABI sceKernelClose();
int PS4ABI sceKernelCloseEport();
int PS4ABI sceKernelCloseEventFlag();
int PS4ABI sceKernelCloseSema();
int PS4ABI sceKernelConvertLocaltimeToUtc();
int PS4ABI sceKernelConvertUtcToLocaltime();
int PS4ABI sceKernelCreateBudget();
int PS4ABI sceKernelCreateEport();
int PS4ABI sceKernelCreateEqueue();
int PS4ABI sceKernelCreateEventFlag();
int PS4ABI sceKernelCreateSema();
int PS4ABI sceKernelDebugGetAppStatus();
int PS4ABI sceKernelDebugGpuPaDebugIsInProgress();
int PS4ABI sceKernelDebugInjectProcessEvent();
int PS4ABI sceKernelDebugRaiseException();
int PS4ABI sceKernelDebugRaiseExceptionOnReleaseMode();
int PS4ABI sceKernelDeleteBudget();
int PS4ABI sceKernelDeleteEport();
int PS4ABI sceKernelDeleteEqueue();
int PS4ABI sceKernelDeleteEventFlag();
int PS4ABI sceKernelDeleteFileEvent();
int PS4ABI sceKernelDeleteHRTimerEvent();
int PS4ABI sceKernelDeleteReadEvent();
int PS4ABI sceKernelDeleteSema();
int PS4ABI sceKernelDeleteTimerEvent();
int PS4ABI sceKernelDeleteUserEvent();
int PS4ABI sceKernelDeleteWriteEvent();
int PS4ABI sceKernelDirectMemoryQuery();
int PS4ABI sceKernelDlsym();
int PS4ABI sceKernelError();
int PS4ABI sceKernelEventLogInit();
int PS4ABI sceKernelEventLogPread();
int PS4ABI sceKernelEventLogWrite();
int PS4ABI sceKernelFchmod();
int PS4ABI sceKernelFcntl();
int PS4ABI sceKernelFdatasync();
int PS4ABI sceKernelFlock();
int PS4ABI sceKernelFstat();
int PS4ABI sceKernelFsync();
int PS4ABI sceKernelFtruncate();
int PS4ABI sceKernelFutimes();
int PS4ABI sceKernelGetAllowedSdkVersionOnSystem();
int PS4ABI sceKernelGetAppInfo();
int PS4ABI sceKernelGetBackupRestoreMode();
int PS4ABI sceKernelGetBackupRestoreModeOfNextBoot();
int PS4ABI sceKernelGetBudget();
int PS4ABI sceKernelGetCompiledSdkVersion();
int PS4ABI sceKernelGetCompiledSdkVersionByPid();
int PS4ABI sceKernelGetCpumode();
int PS4ABI sceKernelGetCpuUsage();
int PS4ABI sceKernelGetCurrentCpu();
int PS4ABI sceKernelGetDataTransferMode();
int PS4ABI sceKernelGetdents();
int PS4ABI sceKernelGetDirectMemorySize();
int PS4ABI sceKernelGetDirectMemoryType();
int PS4ABI sceKernelGetdirentries();
int PS4ABI sceKernelGetEmergencyErrorLog();
int PS4ABI sceKernelGetEventData();
int PS4ABI sceKernelGetEventError();
int PS4ABI sceKernelGetEventFflags();
int PS4ABI sceKernelGetEventFilter();
int PS4ABI sceKernelGetEventId();
int PS4ABI sceKernelGetEventUserData();
int PS4ABI sceKernelGetFsSandboxRandomWord();
int PS4ABI sceKernelGetGPI();
int PS4ABI sceKernelGetGPO();
int PS4ABI sceKernelGetIdPs();
int PS4ABI sceKernelGetIpcPath();
int PS4ABI sceKernelGetModuleInfo();
int PS4ABI sceKernelGetModuleInfoForUnwind();
int PS4ABI sceKernelGetModuleInfoFromAddr();
int PS4ABI sceKernelGetModuleInfoInternal();
int PS4ABI sceKernelGetModuleList();
int PS4ABI sceKernelGetOpenPsIdForSystem();
int PS4ABI sceKernelGetPageTableStats();
int PS4ABI sceKernelGetProcessName();
int PS4ABI sceKernelGetProcessTime();
int PS4ABI sceKernelGetProcessTimeCounter();
int PS4ABI sceKernelGetProcessTimeCounterFrequency();
int PS4ABI sceKernelGetProcParam();
int PS4ABI sceKernelGetProductCode();
int PS4ABI sceKernelGetPrtAperture();
int PS4ABI sceKernelGetResidentFmemCount();
int PS4ABI sceKernelGetResource();
int PS4ABI sceKernelGetResourceLimit();
int PS4ABI sceKernelGetSafemode();
int PS4ABI sceKernelGetSubsysId();
int PS4ABI sceKernelGetSystemExVersion();
int PS4ABI sceKernelGetSystemSwBeta();
int PS4ABI sceKernelGetSystemSwVersion();
int PS4ABI sceKernelGetThreadName();
int PS4ABI sceKernelGettimeofday();
int PS4ABI sceKernelGettimezone();
int PS4ABI sceKernelGetTscFrequency();
int PS4ABI sceKernelGetVddVid();
int PS4ABI sceKernelHasNeoMode();
int PS4ABI sceKernelHwHasOpticalOut();
int PS4ABI sceKernelHwHasWlanBt();
int PS4ABI sceKernelIccControlBDPowerState();
int PS4ABI sceKernelIccGetCpuInfoBit();
int PS4ABI sceKernelIccGetErrLog();
int PS4ABI sceKernelIccGetPowerNumberOfBootShutdown();
int PS4ABI sceKernelIccGetPowerOperatingTime();
int PS4ABI sceKernelIccGetPowerUpCause();
int PS4ABI sceKernelIccGetThermalAlert();
int PS4ABI sceKernelIccGetUSBPowerState();
int PS4ABI sceKernelIccIndicatorBootDone();
int PS4ABI sceKernelIccIndicatorShutdown();
int PS4ABI sceKernelIccNotifyBootStatus();
int PS4ABI sceKernelIccNvsFlush();
int PS4ABI sceKernelIccReadPowerBootMessage();
int PS4ABI sceKernelIccSetBuzzer();
int PS4ABI sceKernelIccSetCpuInfoBit();
int PS4ABI sceKernelInternalHeapPrintBacktraceWithModuleInfo();
int PS4ABI sceKernelInternalMemoryGetModuleSegmentInfo();
int PS4ABI sceKernelIsAddressSanitizerEnabled();
int PS4ABI sceKernelIsAuthenticNeo();
int PS4ABI sceKernelIsCEX();
int PS4ABI sceKernelIsDevKit();
int PS4ABI sceKernelIsGenuineCEX();
int PS4ABI sceKernelIsGenuineDevKit();
int PS4ABI sceKernelIsGenuineN();
int PS4ABI sceKernelIsGenuineTestKit();
int PS4ABI sceKernelIsInSandbox();
int PS4ABI sceKernelIsKratos();
int PS4ABI sceKernelIsNeoMode();
int PS4ABI sceKernelIsStack();
int PS4ABI sceKernelIsTestKit();
int PS4ABI sceKernelJitCreateAliasOfSharedMemory();
int PS4ABI sceKernelJitCreateSharedMemory();
int PS4ABI sceKernelJitGetSharedMemoryInfo();
int PS4ABI sceKernelJitMapSharedMemory();
int PS4ABI sceKernelLoadStartModule();
int PS4ABI sceKernelLseek();
int PS4ABI sceKernelLwfsAllocateBlock();
int PS4ABI sceKernelLwfsLseek();
int PS4ABI sceKernelLwfsSetAttribute();
int PS4ABI sceKernelLwfsTrimBlock();
int PS4ABI sceKernelLwfsWrite();
int PS4ABI sceKernelMapDirectMemory();
int PS4ABI sceKernelMapDirectMemory2();
int PS4ABI sceKernelMapFlexibleMemory();
int PS4ABI sceKernelMapNamedDirectMemory();
int PS4ABI sceKernelMapNamedFlexibleMemory();
int PS4ABI sceKernelMapNamedSystemFlexibleMemory();
int PS4ABI sceKernelMemoryPoolBatch();
int PS4ABI sceKernelMemoryPoolCommit();
int PS4ABI sceKernelMemoryPoolDecommit();
int PS4ABI sceKernelMemoryPoolExpand();
int PS4ABI sceKernelMemoryPoolGetBlockStats();
int PS4ABI sceKernelMemoryPoolReserve();
int PS4ABI sceKernelMkdir();
int PS4ABI sceKernelMlock();
int PS4ABI sceKernelMlockall();
int PS4ABI sceKernelMmap();
int PS4ABI sceKernelMprotect();
int PS4ABI sceKernelMsync();
int PS4ABI sceKernelMtypeprotect();
int PS4ABI sceKernelMunlock();
int PS4ABI sceKernelMunlockall();
int PS4ABI sceKernelMunmap();
int PS4ABI sceKernelNanosleep();
int PS4ABI sceKernelOpen();
int PS4ABI sceKernelOpenEport();
int PS4ABI sceKernelOpenEventFlag();
int PS4ABI sceKernelOpenSema();
int PS4ABI sceKernelPollEventFlag();
int PS4ABI sceKernelPollSema();
int PS4ABI sceKernelPread();
int PS4ABI sceKernelPreadv();
int PS4ABI sceKernelPrintBacktraceWithModuleInfo();
int PS4ABI sceKernelPwrite();
int PS4ABI sceKernelPwritev();
int PS4ABI sceKernelQueryMemoryProtection();
int PS4ABI sceKernelRdup();
int PS4ABI sceKernelRead();
int PS4ABI sceKernelReadTsc();
int PS4ABI sceKernelReadv();
int PS4ABI sceKernelReboot();
int PS4ABI sceKernelReleaseDirectMemory();
int PS4ABI sceKernelReleaseFlexibleMemory();
int PS4ABI sceKernelRename();
int PS4ABI sceKernelReportUnpatchedFunctionCall();
int PS4ABI sceKernelReserveVirtualRange();
int PS4ABI sceKernelRmdir();
int PS4ABI sceKernelSendNotificationRequest();
int PS4ABI sceKernelSetAppInfo();
int PS4ABI sceKernelSetBackupRestoreMode();
int PS4ABI sceKernelSetBudget();
int PS4ABI sceKernelSetCompressionAttribute();
int PS4ABI sceKernelSetCoredumpAppInfo();
int PS4ABI sceKernelSetDataTransferMode();
int PS4ABI sceKernelSetEmergencyErrorLog();
int PS4ABI sceKernelSetEventFlag();
int PS4ABI sceKernelSetFsstParam();
int PS4ABI sceKernelSetGPI();
int PS4ABI sceKernelSetGPO();
int PS4ABI sceKernelSetGpuCu();
int PS4ABI sceKernelSetPhysFmemLimit();
int PS4ABI sceKernelSetProcessName();
int PS4ABI sceKernelSetPrtAperture();
int PS4ABI sceKernelSetSafemode();
int PS4ABI sceKernelSettimeofday();
int PS4ABI sceKernelSetVirtualRangeName();
int PS4ABI sceKernelSetVmContainer();
int PS4ABI sceKernelSignalSema();
int PS4ABI sceKernelSleep();
int PS4ABI sceKernelSpawn();
int PS4ABI sceKernelStat();
int PS4ABI sceKernelStopUnloadModule();
int PS4ABI sceKernelSuspendSystem();
int PS4ABI sceKernelSwitchToBaseMode();
int PS4ABI sceKernelSwitchToNeoMode();
int PS4ABI sceKernelSync();
int PS4ABI sceKernelTerminateSysCore();
int PS4ABI sceKernelTitleWorkaroundIsEnabled();
int PS4ABI sceKernelTitleWorkdaroundIsEnabled();
int PS4ABI sceKernelTriggerEport();
int PS4ABI sceKernelTriggerUserEvent();
int PS4ABI sceKernelTruncate();
int PS4ABI sceKernelUnlink();
int PS4ABI sceKernelUsleep();
int PS4ABI sceKernelUtimes();
int PS4ABI sceKernelUuidCreate();
int PS4ABI sceKernelVirtualQuery();
int PS4ABI sceKernelWaitEqueue();
int PS4ABI sceKernelWaitEventFlag();
int PS4ABI sceKernelWaitSema();
int PS4ABI sceKernelWrite();
int PS4ABI sceKernelWriteSdkEventLog();
int PS4ABI sceKernelWritev();
int PS4ABI sceLibcMspaceCreateForMonoMutex();
int PS4ABI scePthreadAtfork();
int PS4ABI scePthreadAttrDestroy();
int PS4ABI scePthreadAttrGet();
int PS4ABI scePthreadAttrGetaffinity();
int PS4ABI scePthreadAttrGetdetachstate();
int PS4ABI scePthreadAttrGetguardsize();
int PS4ABI scePthreadAttrGetinheritsched();
int PS4ABI scePthreadAttrGetschedparam();
int PS4ABI scePthreadAttrGetschedpolicy();
int PS4ABI scePthreadAttrGetscope();
int PS4ABI scePthreadAttrGetstack();
int PS4ABI scePthreadAttrGetstackaddr();
int PS4ABI scePthreadAttrGetstacksize();
int PS4ABI scePthreadAttrInit();
int PS4ABI scePthreadAttrSetaffinity();
int PS4ABI scePthreadAttrSetcreatesuspend();
int PS4ABI scePthreadAttrSetdetachstate();
int PS4ABI scePthreadAttrSetguardsize();
int PS4ABI scePthreadAttrSetinheritsched();
int PS4ABI scePthreadAttrSetschedparam();
int PS4ABI scePthreadAttrSetschedpolicy();
int PS4ABI scePthreadAttrSetscope();
int PS4ABI scePthreadAttrSetstack();
int PS4ABI scePthreadAttrSetstackaddr();
int PS4ABI scePthreadAttrSetstacksize();
int PS4ABI scePthreadBarrierattrDestroy();
int PS4ABI scePthreadBarrierattrGetpshared();
int PS4ABI scePthreadBarrierattrInit();
int PS4ABI scePthreadBarrierattrSetpshared();
int PS4ABI scePthreadBarrierDestroy();
int PS4ABI scePthreadBarrierInit();
int PS4ABI scePthreadBarrierWait();
int PS4ABI scePthreadCancel();
int PS4ABI scePthreadCondattrDestroy();
int PS4ABI scePthreadCondattrGetclock();
int PS4ABI scePthreadCondattrGetpshared();
int PS4ABI scePthreadCondattrInit();
int PS4ABI scePthreadCondattrSetclock();
int PS4ABI scePthreadCondattrSetpshared();
int PS4ABI scePthreadCondBroadcast();
int PS4ABI scePthreadCondDestroy();
int PS4ABI scePthreadCondInit();
int PS4ABI scePthreadCondSignal();
int PS4ABI scePthreadCondSignalto();
int PS4ABI scePthreadCondTimedwait();
int PS4ABI scePthreadCondWait();
int PS4ABI scePthreadCreate();
int PS4ABI scePthreadDetach();
int PS4ABI scePthreadEqual();
int PS4ABI scePthreadExit();
int PS4ABI scePthreadGetaffinity();
int PS4ABI scePthreadGetconcurrency();
int PS4ABI scePthreadGetcpuclockid();
int PS4ABI scePthreadGetname();
int PS4ABI scePthreadGetprio();
int PS4ABI scePthreadGetschedparam();
int PS4ABI scePthreadGetspecific();
int PS4ABI scePthreadGetthreadid();
int PS4ABI scePthreadJoin();
int PS4ABI scePthreadKeyCreate();
int PS4ABI scePthreadKeyDelete();
int PS4ABI scePthreadMain();
int PS4ABI scePthreadMulti();
int PS4ABI scePthreadMutexattrDestroy();
int PS4ABI scePthreadMutexattrGetkind();
int PS4ABI scePthreadMutexattrGetprioceiling();
int PS4ABI scePthreadMutexattrGetprotocol();
int PS4ABI scePthreadMutexattrGetpshared();
int PS4ABI scePthreadMutexattrGettype();
int PS4ABI scePthreadMutexattrInit();
int PS4ABI scePthreadMutexattrSetkind();
int PS4ABI scePthreadMutexattrSetprioceiling();
int PS4ABI scePthreadMutexattrSetprotocol();
int PS4ABI scePthreadMutexattrSetpshared();
int PS4ABI scePthreadMutexattrSettype();
int PS4ABI scePthreadMutexDestroy();
int PS4ABI scePthreadMutexGetprioceiling();
int PS4ABI scePthreadMutexGetspinloops();
int PS4ABI scePthreadMutexGetyieldloops();
int PS4ABI scePthreadMutexInit();
int PS4ABI scePthreadMutexIsowned();
int PS4ABI scePthreadMutexLock();
int PS4ABI scePthreadMutexSetprioceiling();
int PS4ABI scePthreadMutexSetspinloops();
int PS4ABI scePthreadMutexSetyieldloops();
int PS4ABI scePthreadMutexTimedlock();
int PS4ABI scePthreadMutexTrylock();
int PS4ABI scePthreadMutexUnlock();
int PS4ABI scePthreadOnce();
int PS4ABI scePthreadRename();
int PS4ABI scePthreadResume();
int PS4ABI scePthreadResumeAll();
int PS4ABI scePthreadRwlockattrDestroy();
int PS4ABI scePthreadRwlockattrGetpshared();
int PS4ABI scePthreadRwlockattrGettype();
int PS4ABI scePthreadRwlockattrInit();
int PS4ABI scePthreadRwlockattrSetpshared();
int PS4ABI scePthreadRwlockattrSettype();
int PS4ABI scePthreadRwlockDestroy();
int PS4ABI scePthreadRwlockInit();
int PS4ABI scePthreadRwlockRdlock();
int PS4ABI scePthreadRwlockTimedrdlock();
int PS4ABI scePthreadRwlockTimedwrlock();
int PS4ABI scePthreadRwlockTryrdlock();
int PS4ABI scePthreadRwlockTrywrlock();
int PS4ABI scePthreadRwlockUnlock();
int PS4ABI scePthreadRwlockWrlock();
int PS4ABI scePthreadSelf();
int PS4ABI scePthreadSetaffinity();
int PS4ABI scePthreadSetBesteffort();
int PS4ABI scePthreadSetcancelstate();
int PS4ABI scePthreadSetcanceltype();
int PS4ABI scePthreadSetconcurrency();
int PS4ABI scePthreadSetName();
int PS4ABI scePthreadSetprio();
int PS4ABI scePthreadSetschedparam();
int PS4ABI scePthreadSetspecific();
int PS4ABI scePthreadSingle();
int PS4ABI scePthreadSuspend();
int PS4ABI scePthreadSuspendAll();
int PS4ABI scePthreadTestcancel();
int PS4ABI scePthreadTimedjoin();
int PS4ABI scePthreadYield();
int PS4ABI sched_get_priority_max();
int PS4ABI sched_get_priority_min();
int PS4ABI sched_getparam();
int PS4ABI sched_getscheduler();
int PS4ABI sched_rr_get_interval();
int PS4ABI sched_setparam();
int PS4ABI sched_setscheduler();
int PS4ABI sched_yield();
int PS4ABI select();
int PS4ABI sem_close();
int PS4ABI sem_destroy();
int PS4ABI sem_getvalue();
int PS4ABI sem_init();
int PS4ABI sem_open();
int PS4ABI sem_post();
int PS4ABI sem_timedwait();
int PS4ABI sem_trywait();
int PS4ABI sem_unlink();
int PS4ABI sem_wait();
int PS4ABI send();
int PS4ABI sendfile();
int PS4ABI sendmsg();
int PS4ABI sendto();
int PS4ABI set_phys_fmem_limit();
int PS4ABI setcontext();
int PS4ABI setegid();
int PS4ABI seteuid();
int PS4ABI setgroups();
int PS4ABI setitimer();
int PS4ABI setlogin();
int PS4ABI setpgid();
int PS4ABI setpriority();
int PS4ABI setregid();
int PS4ABI setreuid();
int PS4ABI setrlimit();
int PS4ABI setsid();
int PS4ABI setsockopt();
int PS4ABI settimeofday();
int PS4ABI setuid();
int PS4ABI shm_open();
int PS4ABI shm_unlink();
int PS4ABI shutdown();
int PS4ABI sigaction();
int PS4ABI sigaddset();
int PS4ABI sigaltstack();
int PS4ABI sigdelset();
int PS4ABI sigemptyset();
int PS4ABI sigfillset();
int PS4ABI sigismember();
int PS4ABI siglongjmp();
int PS4ABI signal();
int PS4ABI sigpending();
int PS4ABI sigprocmask();
int PS4ABI sigqueue();
int PS4ABI sigreturn();
int PS4ABI sigsetjmp();
int PS4ABI sigsuspend();
int PS4ABI sigtimedwait();
int PS4ABI sigwait();
int PS4ABI sigwaitinfo();
int PS4ABI sleep();
int PS4ABI socket();
int PS4ABI socketpair();
int PS4ABI stat();
int PS4ABI statfs();
int PS4ABI swapcontext();
int PS4ABI swapon();
int PS4ABI symlink();
int PS4ABI symlinkat();
int PS4ABI sync();
int PS4ABI sysarch();
int PS4ABI sysconf();
int PS4ABI sysctl();
int PS4ABI sysctlbyname();
int PS4ABI sysctlnametomib();
int PS4ABI sysKernelGetManufacturingMode();
int PS4ABI sysKernelGetUpdVersion();
int PS4ABI system();
int PS4ABI tcdrain();
int PS4ABI tcflow();
int PS4ABI tcflush();
int PS4ABI tcgetattr();
int PS4ABI tcgetpgrp();
int PS4ABI tcgetsid();
int PS4ABI tcsendbreak();
int PS4ABI tcsetattr();
int PS4ABI tcsetpgrp();
int PS4ABI tcsetsid();
int PS4ABI truncate();
int PS4ABI umask();
int PS4ABI unlink();
int PS4ABI unlinkat();
int PS4ABI unmount();
int PS4ABI usleep();
int PS4ABI utimes();
int PS4ABI utrace();
int PS4ABI uuidgen();
int PS4ABI vfork();
int PS4ABI wait();
int PS4ABI wait3();
int PS4ABI wait4();
int PS4ABI waitpid();
int PS4ABI write();
int PS4ABI writev();
int PS4ABI sceKernelSandboxPath();
int PS4ABI sceKernelReleaseTraceDirectMemory();
int PS4ABI sceKernelGetLibkernelTextLocation();
int PS4ABI sceKernelSetMemoryPstate();
int PS4ABI __sys_set_uevt();
int PS4ABI sceKernelGetSocPowerConsumption();
int PS4ABI sceKernelDebugGetPauseCount();
int PS4ABI unk_Cq4JEwkS1dU();
int PS4ABI unk_C2ltEJILIGE();
int PS4ABI unk_C49jelxiaVE();
int PS4ABI sceKernelSuspendDirectMemoryRelease();
int PS4ABI sceKernelIccGetHwInfo();
int PS4ABI sceKernelDebugRaiseExceptionWithContext();
int PS4ABI sceKernelMapTraceMemory();
int PS4ABI sceKernelSetNeoModeClock();
int PS4ABI sceKernelGetCoredumpAppInfo();
int PS4ABI unk_EIZbVQs381s();
int PS4ABI pthread_cond_setname_np();
int PS4ABI __sys_test_debug_rwmem();
int PS4ABI sceKernelIccIndicatorStandbyShutdown();
int PS4ABI __sys_namedobj_delete();
int PS4ABI sceKernelDebugAcquireAndUpdateDebugRegister();
int PS4ABI sceKernelAioDeleteRequests();
int PS4ABI sceKernelGetSanitizerNewReplace();
int PS4ABI sceKernelGetCpumodeGame();
int PS4ABI sceKernelLoadStartModuleForSysmodule();
int PS4ABI sceKernelDebugRaiseExceptionWithInfo();
int PS4ABI sceKernelAioSubmitReadCommands();
int PS4ABI unk_HruYl9uBnNo();
int PS4ABI unk_Htwoe5tvNYg();
int PS4ABI unk_H0hj8V6fJwk();
int PS4ABI sceKernelIsGenuineKratosCex();
int PS4ABI __sys_opmc_get_hw();
int PS4ABI unk_JVYt1hlHSvo();
int PS4ABI sceKernelIsMainOnStanbyMode();
int PS4ABI sceKernelNormalizePath();
int PS4ABI sceKernelSetCpumodeGame();
int PS4ABI sceKernelAioWaitRequest();
int PS4ABI sceKernelRtldControl();
int PS4ABI sceKernelIsDebuggerAttached();
int PS4ABI __sys_dynlib_get_info_for_libdbg();
int PS4ABI pthread_cond_reltimedwait_np();
int PS4ABI unk_LJVcP_0wRIk();
int PS4ABI sceKernelIccGetSysEventLog();
int PS4ABI sceKernelEventLogRead();
int PS4ABI sceKernelInternalGetKmemStatistics();
int PS4ABI __sys_opmc_enable();
int PS4ABI sysKernelGetLowerLimitUpdVersion();
int PS4ABI unk_MaCKSOQDtyU();
int PS4ABI sceKernelGetPagingStatsOfAllObjects();
int PS4ABI __sys_resume_internal_hdd();
int PS4ABI sceKernelSuspendProcess();
int PS4ABI sceKernelClearBootReqNotifyCount();
int PS4ABI unk_N94a0T4F4_U();
int PS4ABI sceKernelGetExtLibcHandle();
int PS4ABI sceKernelGetTraceMemoryStats();
int PS4ABI __sys_is_development_mode();
int PS4ABI __sys_opmc_set_hw();
int PS4ABI sceKernelResumeDirectMemoryRelease();
int PS4ABI __sys_dynlib_get_list2();
int PS4ABI sceKernelSetCallRecord();
int PS4ABI sceKernelDebugGetSdkLogText();
int PS4ABI pthread_resume_user_context_np();
int PS4ABI unk_QZ3WWIhtNbE();
int PS4ABI sceKernelGetCpuUsageProc2();
int PS4ABI sceKernelRemoveExceptionHandler();
int PS4ABI sceKernelDebugGetPrivateLogText();
int PS4ABI sceKernelGetKqueueFromEqueue();
int PS4ABI sceKernelGetResidentCount();
int PS4ABI sceKernelGetResourceOne();
int PS4ABI unk_TVB77yVa_RU();
int PS4ABI sceKernelGetCpuUsageAll();
int PS4ABI unk_ToKKfC8tKIk();
int PS4ABI _sceKernelRtldThreadAtexitIncrement();
int PS4ABI unk_T1t68JwPqME();
int PS4ABI __sys_netabort();
int PS4ABI pthread_rwlock_setname_np();
int PS4ABI sceKernelIccGetCountTime();
int PS4ABI sceKernelSetProcessPropertyString();
int PS4ABI sceKernelGetCpuUsageProc();
int PS4ABI unk_VbrTamW6dJs();
int PS4ABI sceKernelTerminateProcess();
int PS4ABI unk_Vo1qeT4JY6I();
int PS4ABI sceKernelSetBaseModeClock();
int PS4ABI sceKernelSetBesteffort();
int PS4ABI _sceKernelSetThreadAtexitReport();
int PS4ABI sceKernelInstallExceptionHandler();
int PS4ABI get_page_table_stats();
int PS4ABI pthread_barrier_setname_np();
int PS4ABI __sys_netgetsockinfo();
int PS4ABI sceKernelSflashGetWritePrio();
int PS4ABI sceKernelAioSubmitWriteCommands();
int PS4ABI __sys_dl_get_list();
int PS4ABI sceKernelSflashSetWritePrio();
int PS4ABI __sys_netgetiflist();
int PS4ABI sceKernelAddGpuExceptionEvent();
int PS4ABI __sys_dl_get_info();
int PS4ABI sceKernelLoadStartModuleInternalForMono();
int PS4ABI sceKernelDirectMemoryQueryForId();
int PS4ABI sceKernelGetHwFeatureInfoForDecid();
int PS4ABI unk_ZotSeVujZaU();
int PS4ABI sceKernelIccControlUSBPowerState();
int PS4ABI sceKernelGetSanitizerNewReplaceExternal();
int PS4ABI sceKernelGetSanitizerMallocReplace();
int PS4ABI unk_b0cryxaTM4k();
int PS4ABI sceKernelProtectDirectMemoryForPID();
int PS4ABI unk_cVC4JXxpb_s();
int PS4ABI unk_cafDxa64sO4();
int PS4ABI sceKernelSetTimezoneInfo();
int PS4ABI sceKernelSetBootReqNotify();
int PS4ABI sceKernelNotifySystemSuspendStart();
int PS4ABI unk_eESTogkCMPE();
int PS4ABI sceKernelSetGameDirectMemoryLimit();
int PS4ABI unk_fFMg_cBHBBY();
int PS4ABI sceKernelAioCancelRequest();
int PS4ABI unk_gpgi_GwE2Is();
int PS4ABI sceKernelAllocateTraceDirectMemory();
int PS4ABI pthread_mutex_init_for_mono();
int PS4ABI sceKernelIccSetCPMode();
int PS4ABI sceKernelGetVrCaptureSize();
int PS4ABI sceKernelNotifySystemSuspendResumeProgress();
int PS4ABI unk_hmqw8GlN_tI();
int PS4ABI __sys_opmc_get_ctr();
int PS4ABI sceKernelGetCpuFrequency();
int PS4ABI __sys_opmc_disable();
int PS4ABI sceKernelResumeProcess();
int PS4ABI sceKernelGetCallRecord();
int PS4ABI sceKernelRaiseException();
int PS4ABI unk_jMuNoBRCPUg();
int PS4ABI __sys_workaround8849();
int PS4ABI sceKernelGetCompiledSdkVersionByPath();
int PS4ABI __sys_osem_open();
int PS4ABI sceKernelProtectDirectMemory();
int PS4ABI sceKernelAioSubmitReadCommandsMultiple();
int PS4ABI sceKernelAioWaitRequests();
int PS4ABI sceKernelAllocateDirectMemoryForMiniApp();
int PS4ABI sceKernelExpectedHddFormat();
int PS4ABI sceKernelKernelHeapUsage();
int PS4ABI __sys_get_proc_type_info();
int PS4ABI __sys_dynlib_get_info2();
int PS4ABI __sys_dynlib_load_prx();
int PS4ABI sceKernelAioInitializeParam();
int PS4ABI __sys_randomized_path();
int PS4ABI sceKernelGetBioUsageAll();
int PS4ABI scePthreadMutexattrInitForInternalLibc();
int PS4ABI sceKernelInternalGetMapStatistics();
int PS4ABI sceKernelIsAllowedToSelectDvdRegion();
int PS4ABI scePthreadSetDefaultstacksize();
int PS4ABI sceKernelGetBootReqNotifyCount();
int PS4ABI unk_opMiSfTJyJo();
int PS4ABI sceKernelIccSetDownloadMode();
int PS4ABI sceKernelAioPollRequests();
int PS4ABI _sceKernelSetThreadAtexitCount();
int PS4ABI __sys_socketex();
int PS4ABI sceKernelGetSanitizerMallocReplaceExternal();
int PS4ABI sceKernelRandomizedPath();
int PS4ABI scePthreadMutexInitForInternalLibc();
int PS4ABI unk_qJnZcaDrZvI();
int PS4ABI sceKernelGetCpuTemperature();
int PS4ABI unk_qtHOLtFWq7Y();
int PS4ABI _sceKernelSetThreadDtors();
int PS4ABI __sys_osem_close();
int PS4ABI sceKernelInternalMemoryGetAvailableSize();
int PS4ABI unk_tFD04DtJY_g();
int PS4ABI pthread_set_defaultstacksize_np();
int PS4ABI pthread_get_specificarray_np();
int PS4ABI sceKernelGetProductStr();
int PS4ABI __sys_socketclose();
int PS4ABI unk_uV_YilcHqAs();
int PS4ABI sceKernelAddResource();
int PS4ABI sceKernelEnableDmemAliasing();
int PS4ABI sceKernelGetModuleListInternal();
int PS4ABI __sys_opmc_set_ctl();
int PS4ABI sceKernelAioInitializeImpl();
int PS4ABI sceKernelIsMainOnStandbyMode();
int PS4ABI sceKernelEnablePthreadObjectCheck();
int PS4ABI __sys_dynlib_get_list_for_libdbg();
int PS4ABI sceKernelEnableDmemAliasing2();
int PS4ABI unk_v_qTlNjF6kE();
int PS4ABI sceKernelGetExecutableModuleHandle();
int PS4ABI sceKernelGetBasicProductShape();
int PS4ABI unk_we5t5V4Fc70();
int PS4ABI get_module_info_list();
int PS4ABI sceKernelInternalMapDirectMemory();
int PS4ABI sceKernelAioSubmitWriteCommandsMultiple();
int PS4ABI unk_x0DQK0TXDk0();
int PS4ABI unk_ys7_x21Qv24();
int PS4ABI sceKernelIccGetBDPowerState();
int PS4ABI __sys_netcontrol();
int PS4ABI __sys_opmc_set_ctr();
int PS4ABI sceKernelGetSocSensorTemperature();
int PS4ABI unk_0NyJTCJWuuk();
int PS4ABI sceKernelInternalMapNamedDirectMemory();
int PS4ABI sceKernelGetMainSocId();
int PS4ABI unk_0yVJNcMqyXk();
int PS4ABI sceKernelIccGetCPMode();
int PS4ABI sceKernelGetAslrStatus();
int PS4ABI sceKernelInternalResumeDirectMemoryRelease();
int PS4ABI unk_1v23w7Rnkic();
int PS4ABI __sys_kqueueex();
int PS4ABI sceKernelGetIdTableCurrentCount();
int PS4ABI sceKernelSetSafemodeDirect();
int PS4ABI sceKernelAioPollRequest();
int PS4ABI unk_2wKjID0iGVM();
int PS4ABI unk_2xg4JXvaJ6k();
int PS4ABI sceKernelNotifyAppStateChanged();
int PS4ABI __sys_namedobj_create();
int PS4ABI unk_3KIbxxVQv84();
int PS4ABI sceKernelAioCancelRequests();
int PS4ABI sceKernelInternalSuspendDirectMemoryRelease();
int PS4ABI sceKernelGetProcessTypeOfBudget();
int PS4ABI sceKernelReserveSystemDirectMemory();
int PS4ABI sceKernelAioDeleteRequest();
int PS4ABI sceKernelPrepareToSuspendProcess();
int PS4ABI unk_58GfHBfwpOY();
int PS4ABI __sys_resume_process();
int PS4ABI __sys_regmgr_call();
int PS4ABI __sys_dl_get_metadata();
int PS4ABI sceKernelIccIndicatorStandbyBoot();
int PS4ABI sceKernelGetPagingStatsOfAllThreads();
int PS4ABI sceKernelGetCurrentFanDuty();
int PS4ABI unk_8KDMful7g1Y();
int PS4ABI _sceKernelRtldThreadAtexitDecrement();
int PS4ABI unk_8PGQdZFioFQ();
int PS4ABI sceKernelMapSanitizerShadowMemory();
int PS4ABI sceKernelDebugReleaseDebugContext();
int PS4ABI sceKernelDeleteGpuExceptionEvent();
int PS4ABI __sys_rdup();
int PS4ABI sceKernelDebugOutText();
int PS4ABI sceKernelPrepareDirectMemorySwap();
int PS4ABI sceKernelAioSetParam();
int PS4ABI sceKernelGetRenderingMode();
int PS4ABI sceKernelGetMemoryPstate();
int PS4ABI sceKernelIccIndicatorStandby();
int PS4ABI unk__XuZD9HV0p4();
int PS4ABI sceKernelGetProcessType();
int PS4ABI sceKernelQueryTraceMemory();
int PS4ABI unk__PNB3tfnD8c();
int PS4ABI sceKernelSetProcessProperty();
int PS4ABI sceKernelPrepareToResumeProcess();
int PS4ABI __sys_suspend_process();
int PS4ABI unk__uPq82VbRMI();
int PS4ABI sem_setname();
int PS4ABI sceKernelAvailableFlexibleMemorySize();
int PS4ABI sceKernelGetCpumode();
int PS4ABI sceKernelSetFsstParam();
int PS4ABI unk_JFpAREOJnag();
int PS4ABI unk_W7Vpt62Xtf4();
int PS4ABI unk_ilQLkGlAjWA();
int PS4ABI sceKernelEnableDmemAliasing();
int PS4ABI sceKernelEnableDmemAliasing();
int PS4ABI sceKernelEnableDmemAliasing();
int PS4ABI sceKernelBacktraceSelf();
int PS4ABI sceKernelRemoveExceptionHandler();
int PS4ABI sceKernelInstallExceptionHandler();
int PS4ABI sceKernelAddGpuExceptionEvent();
int PS4ABI sceKernelDeleteGpuExceptionEvent();
int PS4ABI dup2();
int PS4ABI getrlimit();
int PS4ABI ioctl();
int PS4ABI kill();
int PS4ABI poll();
int PS4ABI pthread_kill();
int PS4ABI sceKernelJitCreateAliasOfSharedMemory();
int PS4ABI sceKernelJitCreateSharedMemory();
int PS4ABI sceKernelJitGetSharedMemoryInfo();
int PS4ABI sceKernelJitMapSharedMemory();
int PS4ABI setrlimit();
int PS4ABI sigaction();
int PS4ABI sigaddset();
int PS4ABI sigdelset();
int PS4ABI sigemptyset();
int PS4ABI sigfillset();
int PS4ABI sigismember();
int PS4ABI sigprocmask();
int PS4ABI sigsuspend();
int PS4ABI socketpair();
int PS4ABI sysconf();
int PS4ABI sysctl();
int PS4ABI sceKernelGetExecutableModuleHandle();
int PS4ABI unk_ClMdHuu_R1A();
int PS4ABI sceKernelGetModuleInfo2();
int PS4ABI sceKernelGetModuleList2();
int PS4ABI sceKernelIsModuleLoaded();
int PS4ABI sceKernelBatchMap();
int PS4ABI sceKernelMapDirectMemory();
int PS4ABI sceKernelMapNamedDirectMemory();
int PS4ABI sceKernelJitCreateAliasOfSharedMemory();
int PS4ABI sceKernelJitCreateSharedMemory();
int PS4ABI sceKernelJitGetSharedMemoryInfo();
int PS4ABI sceKernelJitMapSharedMemory();
int PS4ABI socketpair();
int PS4ABI access();
int PS4ABI chdir();
int PS4ABI dup();
int PS4ABI dup2();
int PS4ABI ioctl();
int PS4ABI lstat();
int PS4ABI pipe();
int PS4ABI poll();
int PS4ABI pthread_get_user_context_np();
int PS4ABI pthread_kill();
int PS4ABI pthread_mutex_setname_np();
int PS4ABI pthread_suspend_user_context_np();
int PS4ABI sceKernelDebugRaiseException();
int PS4ABI sceKernelGetFsSandboxRandomWord();
int PS4ABI sceKernelGetModuleInfoFromAddr();
int PS4ABI sceKernelJitCreateAliasOfSharedMemory();
int PS4ABI sceKernelJitCreateSharedMemory();
int PS4ABI sceKernelJitMapSharedMemory();
int PS4ABI sceKernelOpenSema();
int PS4ABI sigaction();
int PS4ABI sigaddset();
int PS4ABI sigaltstack();
int PS4ABI sigdelset();
int PS4ABI sigemptyset();
int PS4ABI sigfillset();
int PS4ABI sigismember();
int PS4ABI signal();
int PS4ABI sigprocmask();
int PS4ABI sigsuspend();
int PS4ABI socketpair();
int PS4ABI sysconf();
int PS4ABI sceKernelDebugRaiseExceptionWithContext();
int PS4ABI pthread_cond_setname_np();
int PS4ABI pthread_resume_user_context_np();
int PS4ABI sceKernelSetProcessProperty();
int PS4ABI sem_setname();
int PS4ABI ioctl();
int PS4ABI sceKernelIccGetErrLog();
int PS4ABI sceKernelGetSocPowerConsumption();
int PS4ABI sceKernelIccGetHwInfo();
int PS4ABI sceKernelIccGetSysEventLog();
int PS4ABI sceKernelIccGetCountTime();
int PS4ABI unk_gpgi_GwE2Is();
int PS4ABI unk_uV_YilcHqAs();
int PS4ABI sysc_e00();
int PS4ABI sysc_s00();
int PS4ABI sceKernelRemoveExceptionHandler();
int PS4ABI sceKernelInstallExceptionHandler();
int PS4ABI sceKernelRaiseException();
int PS4ABI sceCoredumpAttachMemoryRegion();
int PS4ABI sceCoredumpAttachMemoryRegionAsUserFile();
int PS4ABI sceCoredumpAttachUserFile();
int PS4ABI sceCoredumpConfigDumpMode();
int PS4ABI sceCoredumpDebugTextOut();
int PS4ABI sceCoredumpGetStopInfoCpu();
int PS4ABI sceCoredumpGetStopInfoGpu();
int PS4ABI sceCoredumpGetThreadContextInfo();
int PS4ABI sceCoredumpRegisterCoredumpHandler();
int PS4ABI sceCoredumpSetUserDataType();
int PS4ABI sceCoredumpUnregisterCoredumpHandler();
int PS4ABI sceCoredumpWriteUserData();
int PS4ABI sceCoredumpInternalGetThreadContextInfo();
int PS4ABI sceCoredumpDebugTriggerCoredump();
int PS4ABI sceCoredumpDebugForceCoredumpOnAppClose();
int PS4ABI sceKernelGetOpenPsId();
int PS4ABI accept();
int PS4ABI bind();
int PS4ABI chflags();
int PS4ABI chmod();
int PS4ABI clock_getres();
int PS4ABI clock_gettime();
int PS4ABI close();
int PS4ABI connect();
int PS4ABI creat();
int PS4ABI fchflags();
int PS4ABI fchmod();
int PS4ABI fcntl();
int PS4ABI flock();
int PS4ABI fstat();
int PS4ABI fsync();
int PS4ABI ftruncate();
int PS4ABI futimes();
int PS4ABI getcontext();
int PS4ABI getdents();
int PS4ABI getdirentries();
int PS4ABI getdtablesize();
int PS4ABI getpagesize();
int PS4ABI getpeername();
int PS4ABI getpid();
int PS4ABI getsockname();
int PS4ABI getsockopt();
int PS4ABI gettimeofday();
int PS4ABI htonl();
int PS4ABI htons();
int PS4ABI inet_ntop();
int PS4ABI inet_pton();
int PS4ABI kevent();
int PS4ABI kqueue();
int PS4ABI listen();
int PS4ABI lseek();
int PS4ABI madvise();
int PS4ABI mkdir();
int PS4ABI mlock();
int PS4ABI mlockall();
int PS4ABI mmap();
int PS4ABI mprotect();
int PS4ABI msync();
int PS4ABI munlock();
int PS4ABI munlockall();
int PS4ABI munmap();
int PS4ABI nanosleep();
int PS4ABI ntohl();
int PS4ABI ntohs();
int PS4ABI open();
int PS4ABI pread();
int PS4ABI preadv();
int PS4ABI pthread_attr_destroy();
int PS4ABI pthread_attr_get_np();
int PS4ABI pthread_attr_getdetachstate();
int PS4ABI pthread_attr_getguardsize();
int PS4ABI pthread_attr_getinheritsched();
int PS4ABI pthread_attr_getschedparam();
int PS4ABI pthread_attr_getschedpolicy();
int PS4ABI pthread_attr_getscope();
int PS4ABI pthread_attr_getstack();
int PS4ABI pthread_attr_getstackaddr();
int PS4ABI pthread_attr_getstacksize();
int PS4ABI pthread_attr_init();
int PS4ABI pthread_attr_setdetachstate();
int PS4ABI pthread_attr_setguardsize();
int PS4ABI pthread_attr_setinheritsched();
int PS4ABI pthread_attr_setschedparam();
int PS4ABI pthread_attr_setschedpolicy();
int PS4ABI pthread_attr_setscope();
int PS4ABI pthread_attr_setstack();
int PS4ABI pthread_attr_setstackaddr();
int PS4ABI pthread_attr_setstacksize();
int PS4ABI pthread_barrier_destroy();
int PS4ABI pthread_barrier_init();
int PS4ABI pthread_barrier_wait();
int PS4ABI pthread_barrierattr_destroy();
int PS4ABI pthread_barrierattr_getpshared();
int PS4ABI pthread_barrierattr_init();
int PS4ABI pthread_barrierattr_setpshared();
int PS4ABI pthread_cancel();
int PS4ABI pthread_cleanup_pop();
int PS4ABI pthread_cleanup_push();
int PS4ABI pthread_cond_broadcast();
int PS4ABI pthread_cond_destroy();
int PS4ABI pthread_cond_init();
int PS4ABI pthread_cond_signal();
int PS4ABI pthread_cond_signalto_np();
int PS4ABI pthread_cond_timedwait();
int PS4ABI pthread_cond_wait();
int PS4ABI pthread_condattr_destroy();
int PS4ABI pthread_condattr_getclock();
int PS4ABI pthread_condattr_getpshared();
int PS4ABI pthread_condattr_init();
int PS4ABI pthread_condattr_setclock();
int PS4ABI pthread_condattr_setpshared();
int PS4ABI pthread_create();
int PS4ABI pthread_create_name_np();
int PS4ABI pthread_detach();
int PS4ABI pthread_equal();
int PS4ABI pthread_exit();
int PS4ABI pthread_getconcurrency();
int PS4ABI pthread_getcpuclockid();
int PS4ABI pthread_getname_np();
int PS4ABI pthread_getprio();
int PS4ABI pthread_getschedparam();
int PS4ABI pthread_getspecific();
int PS4ABI pthread_join();
int PS4ABI pthread_key_create();
int PS4ABI pthread_key_delete();
int PS4ABI pthread_mutex_destroy();
int PS4ABI pthread_mutex_getprioceiling();
int PS4ABI pthread_mutex_init();
int PS4ABI pthread_mutex_lock();
int PS4ABI pthread_mutex_setprioceiling();
int PS4ABI pthread_mutex_timedlock();
int PS4ABI pthread_mutex_trylock();
int PS4ABI pthread_mutex_unlock();
int PS4ABI pthread_mutexattr_destroy();
int PS4ABI pthread_mutexattr_getprioceiling();
int PS4ABI pthread_mutexattr_getprotocol();
int PS4ABI pthread_mutexattr_getpshared();
int PS4ABI pthread_mutexattr_gettype();
int PS4ABI pthread_mutexattr_init();
int PS4ABI pthread_mutexattr_setprioceiling();
int PS4ABI pthread_mutexattr_setprotocol();
int PS4ABI pthread_mutexattr_setpshared();
int PS4ABI pthread_mutexattr_settype();
int PS4ABI pthread_once();
int PS4ABI pthread_rename_np();
int PS4ABI pthread_rwlock_destroy();
int PS4ABI pthread_rwlock_init();
int PS4ABI pthread_rwlock_rdlock();
int PS4ABI pthread_rwlock_timedrdlock();
int PS4ABI pthread_rwlock_timedwrlock();
int PS4ABI pthread_rwlock_tryrdlock();
int PS4ABI pthread_rwlock_trywrlock();
int PS4ABI pthread_rwlock_unlock();
int PS4ABI pthread_rwlock_wrlock();
int PS4ABI pthread_rwlockattr_destroy();
int PS4ABI pthread_rwlockattr_getpshared();
int PS4ABI pthread_rwlockattr_gettype_np();
int PS4ABI pthread_rwlockattr_init();
int PS4ABI pthread_rwlockattr_setpshared();
int PS4ABI pthread_rwlockattr_settype_np();
int PS4ABI pthread_self();
int PS4ABI pthread_setcancelstate();
int PS4ABI pthread_setcanceltype();
int PS4ABI pthread_setconcurrency();
int PS4ABI pthread_setprio();
int PS4ABI pthread_setschedparam();
int PS4ABI pthread_setspecific();
int PS4ABI pthread_sigmask();
int PS4ABI pthread_testcancel();
int PS4ABI pthread_yield();
int PS4ABI pwrite();
int PS4ABI pwritev();
int PS4ABI read();
int PS4ABI readv();
int PS4ABI recv();
int PS4ABI recvfrom();
int PS4ABI recvmsg();
int PS4ABI rename();
int PS4ABI rmdir();
int PS4ABI sched_get_priority_max();
int PS4ABI sched_get_priority_min();
int PS4ABI sched_getparam();
int PS4ABI sched_setparam();
int PS4ABI sched_yield();
int PS4ABI select();
int PS4ABI sem_destroy();
int PS4ABI sem_getvalue();
int PS4ABI sem_init();
int PS4ABI sem_post();
int PS4ABI sem_timedwait();
int PS4ABI sem_trywait();
int PS4ABI sem_unlink();
int PS4ABI sem_wait();
int PS4ABI send();
int PS4ABI sendmsg();
int PS4ABI sendto();
int PS4ABI setcontext();
int PS4ABI setsockopt();
int PS4ABI shutdown();
int PS4ABI sleep();
int PS4ABI socket();
int PS4ABI stat();
int PS4ABI swapcontext();
int PS4ABI sync();
int PS4ABI truncate();
int PS4ABI unlink();
int PS4ABI usleep();
int PS4ABI utimes();
int PS4ABI write();
int PS4ABI writev();
