...
static bool decodeNid(const char* name, uint64_t& lid, uint64_t& mid) {
    ...
    if (!loader::decode_nid(&name[12], 1, lid))
        decodeSuccess = false;
    if (!loader::decode_nid(&name[14], 1, mid))
        decodeSuccess = false;
    if (!decodeSuccess) {
...
constexpr uint32_t default_rip_size = 2 * 1024;
