...
bool vmManager::init() {
    ...
    pinfo.userStack = static_cast<uint8_t*>(utl::allocMem(
        nullptr, pinfo.userStackSize, utl::pageProtection::priv, utl::allocationType::reserve));
...
void vmManager::unmapRtMemory(uint8_t* ptr) {
    auto iter = std::find_if(rtPages.begin(), rtPages.end(),
                             [&ptr](const auto& page) { return page.ptr == ptr; });
