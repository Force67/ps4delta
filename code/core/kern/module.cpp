...
#include "amd64/native/native_lift.h"
...
bool smodule::mapImage() {
    ...
    for (uint16_t i = 0; i < elf->phnum; ++i) {
        ...
        if (p->type == PT_LOAD || p->type == PT_SCE_RELRO) {
            ...
            mapSize =
                std::max<uint32_t>(p->vaddr + align_up<uint32_t>(p->memsz, ps4_page), mapSize);
    ...
    info.base = static_cast<uint8_t*>(
        utl::allocMem(nullptr, eight_gb, utl::pageProtection::w, utl::allocationType::reserve));
    ...
    for (uint16_t i = 0; i < elf->phnum; i++) {
        ...
        if (s->type == PT_LOAD || s->type == PT_SCE_RELRO) {
            void* target = elf->type == ET_SCE_EXEC ? /*reinterpret_cast<void *>(s->vaddr)*/
    ...
    if (info.handle == 1) {
        *getAddress<uint32_t>(0x68264) = UINT32_MAX;
        //*getAddress<uint32_t>(0x02FF81) = 0xCCCC;
        //*getAddress<uint32_t>(0x300A2) = 0x9090;
        auto noprange = [&](unsigned int addr, int r, int op) {
            for (int i = 0; i < r; i++) {
                *getAddress<uint8_t>(addr + i) = op;
            }
        };

        noprange(0x30051, 5, 0xCC);
        noprange(0x30097, 5, 0xCC);
        //*getAddress<uint32_t>(0x30051) = 0xCCCC;
        // *getAddress<uint32_t>(0x30097) = 0xCCCC;
        // noprange(0x300A2, 5);
...
bool smodule::resolveImports() {
    ...
    uintptr_t addrBadCall =
        process->getModule("libkernel")->getSymbolFullName("M0z6Dr6TNnM#libkernel#libkernel");
    ...
    for (size_t i = 0; i < numRela; i++) {
        ...
        case R_X86_64_PC32:
            *getAddress<uint32_t>(r->offset) =
                static_cast<uint32_t>(symVal + r->addend - getAddressNPTR<uint64_t>(r->offset));
            break;
