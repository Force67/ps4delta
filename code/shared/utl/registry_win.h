...
class regStream {
public:
  inline explicit regStream(const wchar_t *subKey,
  ...
  inline bool read(const wchar_t *name, uint8_t *data, size_t size) {
    if (key) {
      ...
      return RegQueryValueExW(key, name, nullptr, &type, data,
                              &rsize /*size in bytes*/);
    }
    return false;
  }
  ...
  inline bool write(const wchar_t *name, const uint8_t *data, size_t size) {
    if (key) {
      return RegSetValueExW(key, name, 0, REG_EXPAND_SZ, data,
                            size /*size in bytes*/);
    }
    return false;
  }

  template <typename T> inline bool read(const wchar_t *name, T &val) {
    return read(name, reinterpret_cast<uint8_t *>(&val), sizeof(val));
  }

  template <typename T> bool write(const wchar_t *name, const T &val) {
  ...
  inline bool read(const wchar_t *name, std::basic_string<T> &str) {
    return read(name, reinterpret_cast<uint8_t *>(str.data()),
                str.size() * sizeof(T));
  }
